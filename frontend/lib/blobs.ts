 ...
 * imports too — one interface, three drivers (Vercel Blob or S3-compatible for
 * deploy, filesystem for local dev), chosen by environment. This file exists
 * for one reason: `import "server-only"` (CLAUDE.md §Conventions). It turns
 * importing the store from a client component into a build error, and a package
 * under `node_modules` cannot carry that marker on the app's behalf.
 *
